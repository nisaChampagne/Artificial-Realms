...
• When a skill/attack check is needed, include a roll tag with the ability name: [DICE:d20+3:Wisdom] where +3 is the modifier and Wisdom is the ability. Format: [DICE:spec:AbilityName]
  Examples: [DICE:d20+2:Wisdom], [DICE:d20+1:Strength], [DICE:d20+3:Dexterity], [DICE:d20-1:Intelligence], [DICE:d20+0:Constitution], [DICE:d20+4:Charisma]
  Use D&D 5e DCs: Easy=10, Medium=15, Hard=20, Very Hard=25.
  When the narrative calls for a check, include the tag BEFORE presenting choices so the player rolls before deciding their next action.
  ...
  async _processResponse(raw) {
    ...
    commands.forEach(({ cmd, val }) => {
      switch (cmd) {
        ...
        case 'DICE': {
          // Parse DICE tag: d20+3:Wisdom or just d20+3
          const parts = val.split(':');
          const spec  = parts[0]?.trim().toLowerCase() || 'd20';
          const ability = parts[1]?.trim() || 'Ability';
          diceRequestFromTag = { spec, label: `${ability} Check` };
          break;
        }
        // ── Memory tags ───────────────────────────────────────
        case 'NPC': {
    ...
    // Prefer dice request from DICE tag over text parsing
    const finalDiceRequest = diceRequestFromTag || diceRequest;

    // Helper to trigger the regular skill-check dice prompt (if any)
    const triggerDiceRequest = () => {
      if (!finalDiceRequest) return;
      window.diceSystem.requestRoll(finalDiceRequest.spec, finalDiceRequest.label, (result) => {
        const modifier  = finalDiceRequest.spec.match(/[+-]\d+/)?.[0] || '+0';
        const checkText = `I rolled a ${result.total} (${finalDiceRequest.spec}: ${result.rolls.join('+')}${modifier}) for ${finalDiceRequest.label}.`;
